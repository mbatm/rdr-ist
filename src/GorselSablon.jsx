...
function wrapText(ctx, text, maxW, maxLines=3) {
  ...
  for (const w of words) {
    ...
    if (lines.length >= maxLines-1 && words.indexOf(w) < words.length-1) {
      ...
      if (ctx.measureText(cur+' '+remaining).width > maxW) break
    }
  }
...
async function drawTemplate(ctx,{ bgImg,w,h,baslik,spotBaslik,kategori,tarih,textPos,icon,logo }) {
  ...
  // 3. Gradient overlay — güçlü kontrast
  if (textPos==='bottom') {
  ...
  // 6. kayserim logotype (sağ — daha büyük)
  if (logo) {
  ...
  // Başlık boyutu — daha büyük ve güçlü
...
export default function GorselSablon({ gorselUrl, baslik, spotBaslik, kategori, tarih }) {
  ...
  const render = useCallback(async (fmt=format) => {
    ...
    try {
      ...
      // Preview: max 500px genişlik, DPR dahil
      const pw = Math.min(f.w,500)
  ...
  return (
    <div>
      ...
      <div style={{display:'flex',gap:6,marginBottom:'0.875rem',flexWrap:'wrap'}}>
        {FORMATS.map(f=>{
          ...
          return <button key={f.id} onClick={()=>setFormat(f.id)}
            style={{background:on?'rgba(230,57,70,.15)':'transparent',border:,color:on?'#ff7b7b':'var(--muted)',fontWeight:on?500:400,fontSize:12}}>
            <Ic n={f.ic} size={13}/>{f.label}
            <span style={{marginLeft:4,fontSize:10,color:'var(--muted)',opacity:.7}}>{f.w}×{f.h}</span>
          </button>
        })}
      </div>

      <div style={{display:'flex',gap:6,marginBottom:'0.875rem'}}>
        {[['bottom','layout-navbar-expand','Metin altta'],['top','layout-navbar','Metin üstte']].map(([pos,ic,lbl])=>{
          const on=textPos===pos
          return <button key={pos} onClick={()=>setTextPos(pos)}
            style={{background:on?'rgba(255,255,255,.08)':'transparent',border:,fontWeight:on?500:400,fontSize:12}}>
            <Ic n={ic} size={13}/>{lbl}
          </button>
        })}
      </div>
      ...
      <button onClick={()=>render()} disabled={rendering}
        ...
        {rendering?'2x retina şablon uygulanıyor…':'Şablon Uygula & Önizle (2x kalite)'}
      </button>
      ...
      {preview && <div>
        <img src={preview} alt="2x önizleme"
          style={{width:'100%',borderRadius:'var(--radius-md)',border:'0.5px solid var(--border)',display:'block',marginBottom:'0.75rem'}}/>
        <div style={{fontSize:11,color:'var(--muted)',marginBottom:'0.5rem',display:'flex',alignItems:'center',gap:6}}>
          <Ic n="info-circle" size:11/> PNG indirilir (2x retina · kayıpsız kalite)
        </div>

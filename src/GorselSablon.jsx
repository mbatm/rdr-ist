import { useState, useEffect, useRef, useCallback } from 'react'

// ── LOGO DATA ──────────────────────────────────────────────────────────────
const ICON_SRC  = 'data:image/png;base64,'
const BEYAZ_SRC = 'data:image/png;base64,'

// ── FORMAT TANIMLARI ───────────────────────────────────────────────────────
const FORMATS = [
  { id:'instagram', w:1080, h:1080, label:'Instagram',   ic:'brand-instagram' },
  { id:'facebook',  w:1200, h:630,  label:'Facebook',    ic:'brand-facebook'  },
  { id:'x',         w:1600, h:900,  label:'X',           ic:'brand-x'         },
  { id:'youtube',   w:1280, h:720,  label:'YouTube',     ic:'brand-youtube'   },
]

// ── YARDIMCI: metin sarma ──────────────────────────────────────────────────
function wrapText(ctx, text, maxW, maxLines = 4) {
  if (!text) return []
  const words = text.split(' ')
  const lines = []
  let cur = ''
  for (const w of words) {
    const test = cur ? `${cur} ${w}` : w
    if (ctx.measureText(test).width > maxW && cur) {
      lines.push(cur); cur = w
    } else { cur = test }
    if (lines.length >= maxLines - 1 && cur !== test) break
  }
  if (cur) lines.push(cur)
  return lines.slice(0, maxLines)
}

// ── YARDIMCI: yuvarlak dikdörtgen ─────────────────────────────────────────
function roundRect(ctx, x, y, w, h, r) {
  ctx.beginPath()
  ctx.moveTo(x + r, y)
  ctx.lineTo(x + w - r, y)
  ctx.quadraticCurveTo(x + w, y, x + w, y + r)
  ctx.lineTo(x + w, y + h - r)
  ctx.quadraticCurveTo(x + w, y + h, x + w - r, y + h)
  ctx.lineTo(x + r, y + h)
  ctx.quadraticCurveTo(x, y + h, x, y + h - r)
  ctx.lineTo(x, y + r)
  ctx.quadraticCurveTo(x, y, x + r, y)
  ctx.closePath()
}

// ── YARDIMCI: parlaklık tespiti ────────────────────────────────────────────
function detectBrightness(ctx, w, h, textPos) {
  try {
    const sy = textPos === 'bottom' ? Math.floor(h * 0.45) : Math.floor(h * 0.08)
    const sh = Math.floor(h * 0.38)
    const d  = ctx.getImageData(0, sy, w, sh).data
    let sum = 0
    for (let i = 0; i < d.length; i += 4)
      sum += d[i] * 0.299 + d[i+1] * 0.587 + d[i+2] * 0.114
    return (sum / (d.length / 4)) < 115
  } catch { return true }
}

// ── ANA ÇİZİM FONKSİYONU ──────────────────────────────────────────────────
async function drawTemplate(ctx, { bgImg, w, h, baslik, spotBaslik, kategori, tarih, textPos, icon, logo }) {
  const pad  = Math.round(w * 0.038)
  const barH = Math.round(h * 0.062)

  // 1. Arka plan görseli (cover fit)
  if (bgImg) {
    const scale = Math.max(w / bgImg.width, h / bgImg.height)
    const sw = bgImg.width * scale, sh = bgImg.height * scale
    ctx.drawImage(bgImg, (w - sw) / 2, (h - sh) / 2, sw, sh)
  } else {
    ctx.fillStyle = '#1a2535'; ctx.fillRect(0, 0, w, h)
  }

  // 2. Parlaklık tespiti
  const isDark = detectBrightness(ctx, w, h, textPos)

  // 3. Gradient overlay
  const grad = ctx.createLinearGradient(
    0, textPos === 'bottom' ? h * 0.2 : 0,
    0, textPos === 'bottom' ? h : h * 0.8
  )
  grad.addColorStop(0, 'rgba(0,0,0,0)')
  grad.addColorStop(1, isDark ? 'rgba(0,0,0,0.72)' : 'rgba(0,0,0,0.88)')
  ctx.fillStyle = grad; ctx.fillRect(0, 0, w, h)
  if (!isDark) { ctx.fillStyle = 'rgba(0,0,0,0.18)'; ctx.fillRect(0, 0, w, h) }

  // 4. Kırmızı üst bant
  ctx.fillStyle = '#D63031'; ctx.fillRect(0, 0, w, barH)

  // 5. m simgesi (sol)
  if (icon) {
    const ih = Math.round(barH * 0.70), iw = ih
    ctx.drawImage(icon, pad, (barH - ih) / 2, iw, ih)
  }

  // 6. kayserim logotype (sağ)
  if (logo) {
    const lh    = Math.round(barH * 0.54)
    const ratio = logo.naturalWidth / logo.naturalHeight
    const lw    = Math.round(lh * ratio)
    ctx.drawImage(logo, w - pad - lw, (barH - lh) / 2, lw, lh)
  }

  // 7. Metin bloğu
  const stripeW  = Math.round(w * 0.005)
  const textX    = pad + stripeW + Math.round(w * 0.018)
  const textMaxW = w - textX - pad

  const titleSz  = Math.round(w * 0.0385)
  const spotSz   = Math.round(w * 0.023)
  const lineH    = titleSz * 1.38
  const spotLineH = spotSz * 1.42

  ctx.font = `600 ${titleSz}px -apple-system, system-ui, sans-serif`
  const titleLines = wrapText(ctx, baslik || '', textMaxW, 3)

  ctx.font = `400 ${spotSz}px -apple-system, system-ui, sans-serif`
  const spotLines  = spotBaslik ? wrapText(ctx, spotBaslik, textMaxW, 2) : []

  const titleH     = titleLines.length * lineH
  const gapSpot    = spotLines.length > 0 ? Math.round(w * 0.012) : 0
  const spotH      = spotLines.length * spotLineH
  const blockH     = titleH + gapSpot + spotH
  const stripeH    = blockH + Math.round(h * 0.012)

  const badgeSz    = Math.round(h * 0.036)
  const bottomPad  = Math.round(h * 0.035) + badgeSz + Math.round(h * 0.018)

  const blockY = textPos === 'bottom'
    ? h - bottomPad - blockH
    : barH + Math.round(h * 0.028)

  // Sol kırmızı şerit
  ctx.fillStyle = '#D63031'
  ctx.fillRect(pad, blockY - Math.round(h * 0.004), stripeW, stripeH)

  // Başlık satırları
  ctx.shadowColor = 'rgba(0,0,0,0.92)'; ctx.shadowBlur = isDark ? 8 : 18; ctx.shadowOffsetY = 1
  ctx.fillStyle = '#ffffff'
  ctx.font = `600 ${titleSz}px -apple-system, system-ui, sans-serif`
  titleLines.forEach((line, i) => {
    ctx.fillText(line, textX, blockY + lineH * (i + 1) - lineH * 0.22)
  })

  // Spot başlık
  if (spotLines.length > 0) {
    ctx.font = `400 ${spotSz}px -apple-system, system-ui, sans-serif`
    ctx.fillStyle = 'rgba(255,255,255,0.84)'
    ctx.shadowBlur = isDark ? 5 : 13
    const sy0 = blockY + titleH + gapSpot
    spotLines.forEach((line, i) => {
      ctx.fillText(line, textX, sy0 + spotLineH * (i + 1) - spotLineH * 0.22)
    })
  }

  ctx.shadowColor = 'transparent'; ctx.shadowBlur = 0; ctx.shadowOffsetY = 0

  // 8. Kategori badge + tarih
  const badgeY   = h - badgeSz - Math.round(h * 0.028)
  const badgeFnt = Math.round(badgeSz * 0.46)
  const badgeTxt = (kategori || 'GENEL').toUpperCase()
  ctx.font = `600 ${badgeFnt}px -apple-system, system-ui, sans-serif`
  const badgeTxtW = ctx.measureText(badgeTxt).width
  const badgePadH = Math.round(badgeSz * 0.55)
  const badgePadV = Math.round(badgeSz * 0.66)
  const badgeW   = badgeTxtW + badgePadH * 2

  ctx.fillStyle = '#D63031'
  roundRect(ctx, pad, badgeY, badgeW, badgeSz, Math.round(badgeSz * 0.48))
  ctx.fill()
  ctx.fillStyle = '#ffffff'
  ctx.fillText(badgeTxt, pad + badgePadH, badgeY + badgePadV)

  // Tarih
  if (tarih) {
    ctx.font = `400 ${badgeFnt}px -apple-system, system-ui, sans-serif`
    ctx.fillStyle = 'rgba(255,255,255,0.82)'
    ctx.shadowColor = 'rgba(0,0,0,0.8)'; ctx.shadowBlur = 5
    const dateTxtW = ctx.measureText(tarih).width
    ctx.fillText(tarih, w - pad - dateTxtW, badgeY + badgePadV)
    ctx.shadowBlur = 0
  }
}

// ── COMPONENT ──────────────────────────────────────────────────────────────
const Ic = ({ n, size = 15, style = {} }) =>
  <i className={`ti ti-${n}`} aria-hidden="true" style={{ fontSize: size, ...style }} />

export default function GorselSablon({ gorselUrl, baslik, spotBaslik, kategori, tarih }) {
  const [format,   setFormat]   = useState('instagram')
  const [textPos,  setTextPos]  = useState('bottom')
  const [rendering, setRend]    = useState(false)
  const [preview,  setPreview]  = useState(null)
  const [error,    setError]    = useState(null)
  const [logos,    setLogos]    = useState({ icon: null, logo: null })
  const canvasRef = useRef(null)

  // Logo'ları bir kez yükle
  useEffect(() => {
    const load = src => new Promise(res => {
      const img = new Image(); img.onload = () => res(img); img.onerror = () => res(null); img.src = src
    })
    Promise.all([load(ICON_SRC), load(BEYAZ_SRC)]).then(([icon, logo]) => setLogos({ icon, logo }))
  }, [])

  const render = useCallback(async (fmt = format) => {
    if (!gorselUrl) return
    setRend(true); setError(null); setPreview(null)

    const f = FORMATS.find(x => x.id === fmt)
    const canvas = canvasRef.current
    canvas.width = f.w; canvas.height = f.h
    const ctx = canvas.getContext('2d')

    // Görseli proxy üzerinden yükle (CORS engeli yok)
    const proxyUrl = `/api/gorsel-proxy?url=${encodeURIComponent(gorselUrl)}`
    const bgImg = await new Promise(res => {
      const img = new Image()
      img.crossOrigin = 'anonymous'
      img.onload = () => res(img)
      img.onerror = () => res(null)
      img.src = proxyUrl
    })

    try {
      await drawTemplate(ctx, {
        bgImg, w: f.w, h: f.h,
        baslik, spotBaslik, kategori, tarih,
        textPos, icon: logos.icon, logo: logos.logo,
      })
      // Küçük preview üret (maks 480px genişlik)
      const scaleF = Math.min(1, 480 / f.w)
      const prev = document.createElement('canvas')
      prev.width = Math.round(f.w * scaleF); prev.height = Math.round(f.h * scaleF)
      prev.getContext('2d').drawImage(canvas, 0, 0, prev.width, prev.height)
      setPreview(prev.toDataURL('image/jpeg', 0.88))
    } catch (e) {
      setError('Görsel oluşturulamadı: ' + e.message)
    }
    setRend(false)
  }, [format, gorselUrl, baslik, spotBaslik, kategori, tarih, textPos, logos])

  const download = (fmt) => {
    const f = FORMATS.find(x => x.id === fmt)
    const canvas = canvasRef.current
    // Seçili format farklıysa yeniden çiz
    if (canvas.width !== f.w) {
      render(fmt).then(() => doDownload(fmt))
    } else {
      doDownload(fmt)
    }
  }

  const doDownload = (fmt) => {
    const a = document.createElement('a')
    a.href = canvasRef.current.toDataURL('image/jpeg', 0.93)
    a.download = `kayserim-${fmt}-${Date.now()}.jpg`
    a.click()
  }

  if (!gorselUrl) return (
    <div style={{ padding: '0.875rem', background: 'rgba(255,255,255,0.04)', border: '0.5px solid var(--border)', borderRadius: 'var(--radius-md)', fontSize: 13, color: 'var(--muted)', textAlign: 'center' }}>
      Haber görseli bulunamadı — 1ha'dan gelen haberlerde otomatik uygulanır.
    </div>
  )

  return (
    <div>
      <canvas ref={canvasRef} style={{ display: 'none' }} />

      {/* Format seçimi */}
      <div style={{ display: 'flex', gap: 6, marginBottom: '0.875rem', flexWrap: 'wrap' }}>
        {FORMATS.map(f => {
          const on = format === f.id
          return (
            <button key={f.id} onClick={() => setFormat(f.id)}
              style={{ background: on ? 'rgba(230,57,70,0.15)' : 'transparent', border: `0.5px solid ${on ? 'rgba(230,57,70,0.4)' : 'var(--border)'}`, color: on ? '#ff7b7b' : 'var(--muted)', fontWeight: on ? 500 : 400, fontSize: 12 }}>
              <Ic n={f.ic} size={13} />{f.label}
              <span style={{ marginLeft: 4, fontSize: 10, color: 'var(--muted)', opacity: 0.7 }}>{f.w}×{f.h}</span>
            </button>
          )
        })}
      </div>

      {/* Metin pozisyonu */}
      <div style={{ display: 'flex', gap: 6, marginBottom: '0.875rem' }}>
        {[['bottom', 'ti-layout-navbar-expand', 'Metin altta'], ['top', 'ti-layout-navbar', 'Metin üstte']].map(([pos, ic, lbl]) => {
          const on = textPos === pos
          return (
            <button key={pos} onClick={() => setTextPos(pos)}
              style={{ background: on ? 'rgba(255,255,255,0.08)' : 'transparent', border: `0.5px solid ${on ? 'rgba(255,255,255,0.2)' : 'var(--border)'}`, fontWeight: on ? 500 : 400, fontSize: 12 }}>
              <Ic n={ic} size={13} />{lbl}
            </button>
          )
        })}
      </div>

      {/* Oluştur butonu */}
      <button onClick={() => render()} disabled={rendering}
        style={{ fontWeight: 500, background: 'rgba(230,57,70,0.15)', border: '0.5px solid rgba(230,57,70,0.3)', color: '#ff7b7b', marginBottom: '1rem' }}>
        <Ic n={rendering ? 'loader-2' : 'photo'} size={15} />
        {rendering ? 'Şablon uygulanıyor…' : 'Şablon Uygula & Önizle'}
      </button>

      {/* Hata */}
      {error && (
        <div style={{ background: 'rgba(230,57,70,0.08)', border: '0.5px solid rgba(230,57,70,0.3)', borderRadius: 'var(--radius-md)', padding: '9px 12px', fontSize: 12, color: 'rgba(230,57,70,0.9)', marginBottom: '0.875rem' }}>
          <Ic n="alert-circle" size={13} /> {error}
        </div>
      )}

      {/* Önizleme */}
      {preview && (
        <div>
          <img src={preview} alt="Önizleme"
            style={{ width: '100%', borderRadius: 'var(--radius-md)', border: '0.5px solid var(--border)', display: 'block', marginBottom: '0.75rem' }} />

          {/* İndir butonları */}
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {FORMATS.map(f => (
              <button key={f.id} onClick={() => download(f.id)}
                style={{ fontSize: 12, background: 'rgba(0,212,170,0.1)', border: '0.5px solid rgba(0,212,170,0.3)', color: '#00D4AA' }}>
                <Ic n="download" size={13} />{f.label}
              </button>
            ))}
          </div>
        </div>
      )}
    </div>
  )
}

...
export async function onRequestGet({ request, env }) {
  ...
  try {
    if (act === "tani") {
      ...
      try {
        const cp = rowsOf(await query("SELECT campaign.name, campaign.primary_status, campaign.primary_status_reasons, campaign.bidding_strategy_type, campaign.serving_status, campaign.start_date, campaign.end_date FROM campaign WHERE campaign.status = 'ENABLED'", env));
        sonuc.kampanya_durum = cp.map(r => ({ ad: r.campaign && r.campaign.name, primary: r.campaign && r.campaign.primaryStatus, nedenler: r.campaign && r.campaign.primaryStatusReasons, teklif_strateji: r.campaign && r.campaign.biddingStrategyType, serving: r.campaign && r.campaign.servingStatus, baslangic: r.campaign && r.campaign.startDate, bitis: r.campaign && r.campaign.endDate }));

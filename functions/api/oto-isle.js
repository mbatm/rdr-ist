...
function parseRSS(xml) {
  const getTag = (node, tag) => {
    // CDATA veya düz metin her iki formatı da yakala
    const m = node.match(new RegExp(`<${tag}[^>]*>(?:<!\\[CDATA\\[)?([\\s\\S]*?)(?:\\]\\]>)?<\\/${tag}>`, 'i'))
  ...
  while ((match = re.exec(xml)) !== null) {
    ...
    const link = getTag(node, 'link') || ''
    // ID olarak URL'nin son parçasını kullan
    const id   = link.split('/').pop() || link || String(Date.now() + Math.random())

    items.push({
      source_id:  id,
      source_url: link,
      baslik:     getTag(node, 'title'),
      icerik:     getTag(node, 'description'),
      gorsel:     enc ? enc[1] : '',
      kategori:   getTag(node, 'category') || 'Genel',
      tarih_iso:  new Date(getTag(node, 'pubDate') || Date.now()).toISOString(),
    })
  }

function parseRSS(xml) {
  ...
  for (const m of xml.matchAll(/<item>([\s\S]*?)<\/item>/gi)) {
    ...
    // Enclosure: type'a göre görsel veya video
    const encMatch = node.match(/<enclosure[^>]*url="([^"]*)"[^>]*type="([^"]*)"/)
    let gorsel = '', video = ''
    if (encMatch) {
      const [, url, type] = encMatch
      if (type.startsWith('video/')) video = url
      else gorsel = url
    }

    // Görsel yoksa description içindeki img'yi dene
    if (!gorsel) {
      const imgM = node.match(/<img[^>]*src="([^"]*)"/)
      if (imgM) gorsel = imgM[1]
    }

    if (bas.length > 5) items.push({
      source_id: id, source_url: link, baslik: bas, icerik,
      gorsel, video, kategori: kat,
